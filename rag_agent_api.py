"""
TelcoOps Copilot Smart Agent - v3.0 (Self-Learning Edition)

Những gì code cũ (v2.0) CHƯA làm được (vì sao chưa phải là "tự học"):
  - Router (SQL vs RAG) luôn gọi LLM để quyết định -> không hề "học" gì từ
    những lần trả lời trước, tốn tiền/tốn thời gian như nhau mãi mãi.
  - Không có cơ chế feedback: câu trả lời đúng/sai không được lưu lại,
    không có gì để hệ thống "nhớ" và cải thiện theo thời gian.
  - Vector Store (FAISS) chỉ build 1 lần từ file JSON gốc, không có cách
    nạp thêm tri thức mới (tài liệu mới, câu trả lời đã được xác nhận đúng,
    hoặc bản sửa lỗi từ kỹ sư) mà không phải xóa index cũ và build lại từ đầu.
  - Không log lại lịch sử hội thoại -> không có dữ liệu để huấn luyện/tinh
    chỉnh bất cứ thứ gì trong tương lai.

Những gì bản v3.0 này BỔ SUNG để trở thành "tự học" thực sự:
  1. LOGGING: mọi câu hỏi + câu trả lời + nguồn dữ liệu đều được ghi vào
     bảng `conversation_log` trong SQLite.
  2. FEEDBACK LOOP: endpoint /api/feedback cho phép kỹ sư đánh giá
     (đúng/sai) hoặc gửi bản sửa (correction) cho 1 câu trả lời cụ thể.
       - Nếu đánh giá "đúng" (up) -> cặp (câu hỏi, câu trả lời) được thêm
         thẳng vào FAISS như một "tri thức đã được xác nhận" (verified
         knowledge), giúp lần sau hỏi câu tương tự sẽ được trả lời nhanh,
         nhất quán và không cần gọi LLM tổng hợp lại từ đầu.
       - Nếu đánh giá "sai" (down) kèm correction -> bản sửa được nạp vào
         FAISS với độ ưu tiên cao (source = "expert_correction") để ghi đè
         về mặt ngữ nghĩa lên tri thức cũ bị sai.
  3. SELF-LEARNING ROUTER: mỗi lần feedback được xác nhận, cặp
     (câu hỏi, loại nguồn dữ liệu đúng) được lưu vào bảng
     `routing_training_data`. Khi đã đủ dữ liệu, hệ thống tự động huấn
     luyện lại một classifier nhẹ (TF-IDF + Logistic Regression) để tự
     định tuyến SQL/RAG mà KHÔNG cần gọi LLM nữa -> càng dùng nhiều,
     router càng chính xác và càng rẻ/nhanh hơn. LLM chỉ được dùng làm
     "giáo viên" (teacher) dự phòng khi classifier chưa đủ tự tin.
  4. VERIFIED-ANSWER CACHE: trước khi định tuyến, hệ thống tìm trong kho
     tri thức "đã được xác nhận" xem có câu hỏi nào gần giống không. Nếu
     độ tương đồng đủ cao, trả lời ngay từ cache đã học được, không cần
     gọi lại pipeline SQL/RAG/LLM.
  5. INCREMENTAL INGESTION: endpoint /api/ingest cho phép nạp thêm tài
     liệu mới vào FAISS bằng add_texts() (append), không cần xóa và build
     lại toàn bộ index.
...
# ---------------------------------------------------------------------------
# TEXT-TO-SQL THẬT: bản cũ luôn chạy 1 câu SQL cứng
# (SELECT ... FROM cell_kpi LIMIT 5) bất kể câu hỏi là gì, và không bao giờ
# đụng tới bảng alarm_logs. Phần dưới đây sinh SQL động theo đúng câu hỏi.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Ghi log hội thoại (nền tảng cho mọi thứ "tự học" phía sau)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1) VERIFIED-ANSWER CACHE: học trực tiếp câu hỏi <-> câu trả lời đã đúng
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2) SELF-LEARNING ROUTER: TF-IDF + Logistic Regression, huấn luyện lại
#    tự động từ dữ liệu feedback đã được xác nhận, thay dần cho việc luôn
#    phải gọi LLM để định tuyến.
# ---------------------------------------------------------------------------
